/*
 ...
 to delete the terminator from tuple spaces.
...
int  _activate_space( id, workerId )
    int   id;
    char *workerId;
{
    int     sock;
    u_short this_op = TSH_ADVANCED_OP_ACTIVATE;
    sng_map *link_pt;
    tsh_put_it out;
    tsh_put_ot in;
    ...
    {
            perror("_activate_space: get_socket error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
    if (!do_connect(sock, inet_addr(link_pt->cpu),
            link_pt->port))
    {
            perror("_activate_space: TSH connection error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    if (!writen(sock, (char *)&this_op, sizeof(u_short)))
    {
            perror("_activate_space: Op code send error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    ...
    {
            perror("_activate_space: Length send error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
                            /* read status */
    ...
    {
            perror("_activate_space: read status error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
