...
int  _cleanup_queue( id, workerId )
    int   id;
    char *workerId;
{
    int     sock;
    u_short this_op = TSH_ADVANCED_OP_CLEANUP_QUEUE;
    sng_map *link_pt;
    tsh_put_it out;
    tsh_put_ot in;
    ...
    {
            perror("_cleanup_queue: get_socket error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
    if (!do_connect(sock, inet_addr(link_pt->cpu),
            link_pt->port))
    {
            perror("_cleanup_queue: TSH connection error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    if (!writen(sock, (char *)&this_op, sizeof(u_short)))
    {
            perror("_cleanup_queue: Op code send error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    ...
    {
            perror("_cleanup_queue: Length send error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
                            /* read status */
    ...
    {
            perror("_cleanup_queue: read status error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
