/*
 ...
 to set tokens into tuple spaces.
...
int  _set_token( id, tpname, tpvalue, tpsize )
    int   id, tpsize;
    char *tpname;
    char *tpvalue;
{
    int     sock;
    u_short this_op = TSH_ADVANCED_OP_SET_TOKEN;
    sng_map *link_pt;
    tsh_put_it out;
    tsh_put_ot in;
    ...
    {
            perror("_set_token: get_socket error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
    if (!do_connect(sock, inet_addr(link_pt->cpu),
            link_pt->port))
    {
            perror("_set_token: TSH connection error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    if (!writen(sock, (char *)&this_op, sizeof(u_short)))
    {
            perror("_set_token: Op code send error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
    ...
    {
            perror("_set_token: Length send error\n") ;
            close(sock);
            return(TSPUT_ER);
    }
                            /* send tuple to TSH */
    ...
    {
            perror("_set_token: Value send error\n");
            close(sock);
            return(TSPUT_ER);
    }
                            /* read status */
    ...
    {
            perror("_set_token: read status error\n") ;
            close(sock);
            return(TSPUT_ER) ;
    }
