/*====================================================================*/
/*  Subroutine   cnf_xdr_fputs()                                      */
/*      cnf_xdr_fputs() - translates a line to it's external data     */
/*	representation and sends it to file from buffer (starting     */
/*	at address xdr_buff). Bufsize is the number of bytes to       */
/*	send. Id is the index into cnf_map global data structure      */
/*	where file pointer is stored. RETURNS: status.		      */
...
int cnf_xdr_fputs(id, buf, bufsize, e_type)
int id;
char *buf;
int bufsize;
int e_type;
{
        ...
        status = cnf_fputs(id, buf, bufsize);
        xdr_destroy(&xdrs);
        free(xdr_buf);
        return(status);
}
