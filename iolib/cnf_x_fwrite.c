/*====================================================================*/
/*  Subroutine   cnf_xdr_fwrite()                                     */
/*      cnf_xdr_fwrite() - translates a 'record` to it's external     */
/*	data representation and sends it to file from buffer 	      */
/*	(starting at address xdr_buff).	Bufsize is the number of      */
/*	bytes to send. Id is the index into cnf_map global data       */
/*	structure where file pointer is stored. RETURNS: status.      */
...
int cnf_xdr_fwrite(id, buf, bufsize, nitems, e_type)
int id;
char *buf;
int bufsize;
int nitems;
int e_type;
{
        ...
        status = cnf_fwrite(id, xdr_buf, bufsize, nitems);
        xdr_destroy(&xdrs);
        free(xdr_buf);
        return(status);
}
