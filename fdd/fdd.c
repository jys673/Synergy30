...
void initFdd()
{
	...
	if (gethostlist(dacport, &host_list, &host_end, &clist_cnt)) {
		...
		{
		    printf(" FDD error: Remote host lookup failure.\n");
		    exit(1);
		}
		// Setup detection list with cidports. 
		p_q->cidport = cidport;
		p_q->fault_flag = 0;
		p_q = p_q->next;
	    }
        ...
        status = 
	  gethandlerlist(dacport, &handler_list, &handler_end, &hlist_cnt);
        if (!status) {
            printf(" FDD error: Failed to get handler list.\n");
	    fflush(stdout);
	    fflush(stdout);
            exit(1);
        }
...
//		else printf(" FDD. Working host (%u) pinged. \n", p_q->hostid);
	    }
...
printf(" FDD: -- Automatic Tuple Recovery: TShost(%u) id(%s)\n", p_q->host,
		p_q->mapid);
...
int procid;
{
	...
	if ((sock = get_socket()) == -1) exit(-1);
	    ...
	    // printf("FDD checking appid(%s) pid(%d) on (%ul)\n", out.appid, out.proc_id, hostid); 
	...
	printf(" FDD. Cannot connect to CID? \n");
	close(sock);
	return(2);
}
